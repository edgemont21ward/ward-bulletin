...
/* ---------------------------------------------------------------------
 * ONE-OFF SPELLING REPAIR
 *
 * The Stake Business header was typed "STAKE BUISNESS" on the Template
 * tab, so every week created from it inherited the typo — 9/13, 9/20
 * and 9/27 all carry it, while 8/30 and 9/6, written before the
 * Template picked it up, spell it correctly.
 *
 * It isn't only cosmetic. hasStakeBusinessContent_ finds that section
 * by looking up the exact label "Stake Business" (see findLabelIndex_),
 * so on a misspelt tab the section is never found, and the heading
 * stays off the printed bulletin however much is filled in underneath
 * it. That's why "Presented By:" appeared to do nothing.
 *
 * Safe to run more than once: a second run finds nothing left to fix.
 * ------------------------------------------------------------------ */


/**
 * Rewrites "buisness" as "business" everywhere it appears in the
 * spreadsheet, across every tab, matching the case it was written in
 * ("BUISNESS" -> "BUSINESS", "Buisness" -> "Business"). Only cells that
 * actually contain the typo are written back.
 *
 * Returns a list of "<tab>!<cell>: <before> -> <after>" strings
 * describing what it changed, so the caller can show exactly what was
 * touched rather than just a count.
 */
function fixBusinessSpelling_() {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var r = 0; r < values.length; r++) {
      for (var c = 0; c < values[r].length; c++) {
        var cell = values[r][c];
        if (typeof cell !== 'string' || !/buisness/i.test(cell)) continue;

        var corrected = correctBusinessSpelling_(cell);
        // Row/column are 1-based, and getDataRange() can start below
        // row 1, so offset by where the range actually begins.
        var target = sheet.getRange(range.getRow() + r, range.getColumn() + c);
        target.setValue(corrected);
        changes.push(sheet.getName() + '!' + target.getA1Notation() +
          ': "' + cell + '" -> "' + corrected + '"');
      }
    }
  }
...
/** "BUISNESS" -> "BUSINESS", "Buisness" -> "Business", "buisness" -> "business". */
function correctBusinessSpelling_(text) {
  return String(text).replace(/buisness/gi, function (match) {
    if (match === match.toUpperCase()) return 'BUSINESS';
    if (match.charAt(0) === match.charAt(0).toUpperCase()) return 'Business';
    return 'business';
  });
}
...
/** Menu action. */
function fixBusinessSpellingFromMenu() {
  var changes = fixBusinessSpelling_();
  if (!changes.length) {
    SpreadsheetApp.getUi().alert(
      'Nothing to fix',
      'No tab spells it "buisness" — every Stake Business heading is already correct.',
      SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  // Re-render the dropdowns: a corrected header can change which rows
  // the label matching in refreshDropdowns_ recognizes.
  refreshDropdowns_();
  SpreadsheetApp.getUi().alert(
    'Fixed ' + changes.length + (changes.length === 1 ? ' cell' : ' cells'),
    changes.join('\n'),
    SpreadsheetApp.getUi().ButtonSet.OK);
}
